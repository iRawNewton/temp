...
if (isset($_POST['submit']))
{
	...
	if (!empty ($facultyID) || !empty ($username) || !empty ($dob) || !empty ($gender) || !empty ($designation) || !empty ($department) || !empty ($email) || !empty ($phone_no) || !empty ($nationality) || !empty ($address))
    // if (!empty ($username))
	{
		$insert = "insert into faculty values 
		('$facultyID','$username',STR_TO_DATE('$dob', '%d-%m-%Y'),'$gender','$designation','$department','$email','$phone_no','$nationality','$address')";
		...
		{
			echo "<script>alert('Cannot insert duplicate record');</script>";
		}	
	}
	else
	{
		echo "<script>alert('Error occured in connecting to database.');</script>";
	}
}
...
if( isset($_POST['searchBTN']) )
{
    ...
    $sql = "SELECT * FROM faculty WHERE f_ID  ='$search_num'";
...
if (isset($_POST['updateBTN']))
{
	...
	if (!empty ($facultyID) || !empty ($username) || !empty ($dob) || !empty ($gender) || !empty ($designation) || !empty ($department) || !empty ($email) || !empty ($phone_no) || !empty ($nationality) || !empty ($address))
	{
		$update = "update faculty set name='$username',  dob=STR_TO_DATE('$dob', '%d-%m-%Y'), gender='$gender', designation='$designation', department='$department', email='$email', mobile_no='$phone_no', nationality='$nationality', address='$address' where f_ID= '$facultyID'";
...
{
    $facultyID = $_POST['facultyID'];
    $sql = "DELETE FROM faculty WHERE f_ID ='$facultyID'";
    $result = mysqli_query($cn, $sql);
...
if (isset($_POST['printBTN']))
{
    ...
    $name = $_POST['name'];
    require('vendor/autoload.php');
    $res=mysqli_query($cn,"select * from faculty where f_ID='$facultyID'");
    ...
    {        
        ...
        <h2 class="h1-responsive font-weight-bold text-center cuscenter"><u>Faculty Information</u></h2>
                <div class="container">
        ...
        {
            $html.='<tr> 
                        <td class="partleft">Faculty ID </> 
                        <td>:</td>
                        <td class="partleft">'.$row['f_ID'].'</td>
                    </tr>
                    <tr>
                        <td class="partleft">Faculty Name </>
                        <td>:</td>
                    ...
                    <tr>
                        <td class="partleft">Designation </>
                        <td>:</td>
                        <td class="partleft">'.$row['designation'].'</td></tr>
                    <tr>
                        <td class="partleft">Department </>
                        <td>:</td>
                        <td class="partleft">'.$row['department'].'</td></tr>
                    <tr>
                        <td class="partleft">E-mail </>
                        <td>:</td>
                        <td class="partleft">'.$row['email'].'</td></tr>
                    <tr>
                        <td class="partleft">Phone No. </>
                        <td>:</td>
                        <td class="partleft">'.$row['mobile_no'].'</td></tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>:</td>
                        <td class="partleft">'.$row['address'].'</td></tr>
...
<body>
    <form name="faculty" id="facultyFORM" method="POST">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <ul class="navbar-nav ml-auto">
                    ...
                    <li class="nav-item active">
                        <a class="nav-link" href="#">Faculty Form <span class="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
        ...
        <!--Section heading-->
        <section class="mb-0">
            <h2 class="h1-responsive font-weight-bold text-center mt-4 mb-0">Faculty Information</h2>
        </section>
        <!-- Material input -->
        <section class="mb-5">
            <div class="container">
            ...
            <div class="container">
                <div class="row row1 no-gutters mx-1 mb-4">
                    ...
                    <div class="md-form col-md-5 mx-auto text-center">
                        <input type="text" id="facultyID" class="form-control" name="facultyID"  value="<?= $facultyID ?>" <?= $enrollText ?> >
                    ...
                    <div class="md-form col-md-3 mx-auto">
                        <input type="text" id="designation" class="form-control" name="designation" value="<?= $designation ?>" data-toggle="modal" data-target="#exampleModalCenter1" autocomplete="off">
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            ...
                            <div class="modal-body">
                                <!-- ... -->
                                <select class="browser-default custom-select"  id="selectID1">
                            ...
                            <div class="modal-footer">
                                <button class="btn btn-success btn-sm" type="button" onclick='selectTag1()' name="modalSelectBtn">Select</button>
                                <button type="button" id="cancel1" class="btn btn-danger btn-sm" data-dismiss="modal">Close</button>
                            </div>
                            </div>
                        </div>
                        </div>
                    ...
                    <div class="md-form col-md-5 mx-auto">
                        <input type="text" id="department" class="form-control" name="department" value="<?= $department ?>"  data-toggle="modal" data-target="#exampleModalCenter" autocomplete="off">
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            ...
                            <div class="modal-body">
                                ...
                                <select class="browser-default custom-select" name="courseOpt" id="selectID">
                                    <option value="">Choose your option</option>
                                    <?php
                                        $sql = "SELECT distinct dept FROM department";
                                        $result = mysqli_query($cn, $sql);
                    ...
                    <div class="md-form col-md-5 mx-auto">
                        <input type="text" id="phone" class="form-control" name="phone_no" value="<?= $phone_no ?>">
    ...
    <script type="text/javascript">
        ...
        function selectTag()
        {
            ...
            document.getElementById("department").value = x;
            document.getElementById("cancel").click();
        }
        // }exampleModalCenter1
        function selectTag1()
        {
            var x = document.getElementById("selectID1").value;
            document.getElementById("designation").value = x;
            document.getElementById("cancel1").click();
            // if (x=="Faculty")
            // {
            //     document.getElementById("department").value = "Faculty";
            //     document.getElementById("department").style.readonly=true;
            // }
            // else
            // {
            //     document.getElementById("department").style.readonly=false;
            // }
        }
    </script>
